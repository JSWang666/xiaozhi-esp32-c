...
**已修**：`circular_strip_on_state` / `gpio_led_on_state` 调用 `CircularStrip::OnStateChanged(DeviceState, bool)` 与 `GpioLed::OnStateChanged(DeviceState, bool)`；无参 `OnStateChanged()` 仍从 `Application` 取状态并转发到该重载。`GpioLed` 在 Listening 分支使用传入的 `is_voice_detected`，与 C 侧 `led_ops_t` 一致。

#### Bug 6：`protocol_cpp_bridge.cc` 指针寿命

**已修**：`OnIncomingAudio` 在调用 C 回调前将 `packet->payload` **拷贝**到栈上 `std::vector<uint8_t>`，再填 `protocol_audio_packet_t`，避免回调返回后 `unique_ptr` 释放导致 C 端悬垂指针。
...
原板子的 boot 长按/单击在 `kDeviceStateStarting` 时调 `EnterWifiConfigMode()`。新 C 代码应调用 **`board_enter_wifi_config_mode(board_get_instance())`** 或 **`app_enter_wifi_config_mode(app_get_context())`**（`app_c_api.h`，与板级 API 等价委托）。若板子仍写 `return` 而不调用上述 API，配网入口仍会失效。
...
- `display_ops_t` 有 `clear_chat_messages` 字段，**已补** `display_clear_chat_messages()` 内联（`display.h`）。
- `display_ops_t` 没有 `set_theme`，C 端无法切换主题（仍为后续工作）。
- ~~`display_cpp_bridge.cc::disp_lock/unlock` 空实现~~（**已修**：桥接到 `Display::Lock`/`Unlock`）。
...
### P4 - C API 缺口
8. ~~暴露 Wi-Fi 配网入口给 C~~（**已修**：`app_enter_wifi_config_mode()` 在 `app_c_api.h`，委托 `board_enter_wifi_config_mode(board_get_instance())`；纯 C 板可只包含 `app_c_api.h`）
9. ~~`display_ops_t` 的 `lock`/`unlock` 接 LVGL~~（**已修**：`display_cpp_bridge.cc` 调用 `Display::Lock`/`Unlock`；`display.h` 增加 `display_lock`/`display_unlock` 与 `display_clear_chat_messages` 内联）
10. ~~`led_cpp_bridge` 传递 state/voice~~（**已修**，见 Bug 5）

**说明**：`board_enter_wifi_config_mode` 仍要求底层 `Board*` 可 `dynamic_cast` 为 `WifiBoard`；`DualNetworkBoard` 等组合板若未扩展该 API，仍可能返回 `ESP_ERR_NOT_SUPPORTED`，需后续单独处理。
