...
| 板子 | 错误类型 |
|---|---|
| `lichuang-dev` | 缺 `#include "led/led.h"`，`ES8311_CODEC_DEFAULT_ADDR` 未定义 |
| `esp-box-3` | 缺 `device_state.h`、`esp_lcd_panel_ops.h`、ES8311/ES7210 头文件 |
| `kevin-sp-v3-dev` | 用了不存在的类型 `board_btn_handle_t`（应是 `board_btn_t*`） |
| `xingzhi-metal-1.54-wifi` | `cst816x.h` 还是 C++ 文件，被 `.c` 文件包含会失败 |
| `waveshare/esp32-s3-lcd-0.85` | `led_strip_control.h` 是 C++ 类，与 `.c` 不兼容 |

抽样编译 5 个板子里 3 个直接挂掉 → 推断**全部未验证板子里大约一半会有类似头文件/类型错误**。
...
### P3 - 编译失败的板子需逐一修
7. `lichuang-dev`、`esp-box-3`、`kevin-sp-v3-dev`、`xingzhi-metal-1.54-wifi`、`waveshare/esp32-s3-lcd-0.85`
