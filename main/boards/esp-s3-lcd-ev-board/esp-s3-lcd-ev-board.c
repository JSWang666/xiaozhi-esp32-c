...
static void init_codec_i2c(s3_lcd_ev_ctx_t *ctx)
{
    ...
    esp_io_expander_new_i2c_tca9554(ctx->codec_i2c_bus, 0x20, &ctx->expander);
...
static void init_buttons(s3_lcd_ev_ctx_t *ctx)
{
...
static void init_display(s3_lcd_ev_ctx_t *ctx)
{
...
{
    (void)self;
    return "esp-s3-lcd-ev-board";
}

static void *get_led(board_desc_t *self)
{
    s3_lcd_ev_ctx_t *ctx = (s3_lcd_ev_ctx_t *)self;
    if (!ctx->led) {
...
{
    return ((s3_lcd_ev_ctx_t *)self)->display;
}
...
{
    s3_lcd_ev_ctx_t *ctx = calloc(1, sizeof(s3_lcd_ev_ctx_t));
    if (!ctx) return NULL;
