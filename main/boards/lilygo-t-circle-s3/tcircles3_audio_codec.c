...
{
    tcircles3_codec_t *ctx = (tcircles3_codec_t *)codec;
...
static const audio_codec_ops_t tcircles3_ops = {
    .read = codec_read,
    .write = codec_write,
...
{
    tcircles3_codec_t *ctx = calloc(1, sizeof(tcircles3_codec_t));
    if (!ctx) return NULL;

    audio_codec_t *codec = &ctx->base;
    codec->ops = &tcircles3_ops;
    codec->duplex = true;
