...
static void on_asr_click(void *ud)
{
    ...
    app_context_t *app = app_get_context();
    if (!app) return;
    app_wake_word_invoke(app, "\xe4\xbd\xa0\xe5\xa5\xbd\xe5\xb0\x8f\xe6\x99\xba");
}
...
static void init_display_i2c(esp32_bread_ctx_t *ctx)
{
...
static void init_ssd1306_display(esp32_bread_ctx_t *ctx)
{
...
static void init_buttons(esp32_bread_ctx_t *ctx)
{
...
static const char *ebb_get_board_type(board_desc_t *self)
{
    (void)self;
    return BOARD_TYPE;
}

static void *ebb_get_led(board_desc_t *self)
{
    esp32_bread_ctx_t *ctx = (esp32_bread_ctx_t *)self;
    if (!ctx->led) {
...
static void *ebb_get_audio_codec(board_desc_t *self)
{
    esp32_bread_ctx_t *ctx = (esp32_bread_ctx_t *)self;
    if (!ctx->codec) {
...
static void *ebb_get_display(board_desc_t *self)
{
    esp32_bread_ctx_t *ctx = (esp32_bread_ctx_t *)self;
    return ctx->display;
}

static void ebb_destroy(board_desc_t *self)
{
    esp32_bread_ctx_t *ctx = (esp32_bread_ctx_t *)self;
...
{
    esp32_bread_ctx_t *ctx = calloc(1, sizeof(esp32_bread_ctx_t));
    if (!ctx) return NULL;

    ctx->base.kind = BOARD_KIND_WIFI;
    ctx->base.get_board_type = ebb_get_board_type;
    ctx->base.get_led = ebb_get_led;
    ctx->base.get_audio_codec = ebb_get_audio_codec;
    ctx->base.get_display = ebb_get_display;
    ctx->base.destroy = ebb_destroy;
