...
static void init_axp2101(i2c_master_bus_handle_t bus)
{
    ...
    i2c_write_reg(dev, 0x94, (3300 - 500) / 100);
    i2c_write_reg(dev, 0x95, (3300 - 500) / 100);
    i2c_write_reg(dev, 0x90, 0x0F);
    i2c_write_reg(dev, 0x64, 0x02);
    i2c_write_reg(dev, 0x61, 0x02);
    i2c_write_reg(dev, 0x62, 0x0A);
    i2c_write_reg(dev, 0x63, 0x01);

    /* Display reset via ALDO3 */
    uint8_t rd[2] = {0x90, 0};
    i2c_master_transmit_receive(dev, &rd[0], 1, &rd[1], 1, -1);
    uint8_t reg90 = rd[1];
    i2c_write_reg(dev, 0x90, reg90 | 0x04);
...
static void init_display(board_ctx_t *ctx)
{
    ...
    ctx->display = spi_lcd_display_create(panel_io, panel,
        LCD_H_RES, LCD_V_RES, DISPLAY_OFFSET_X, DISPLAY_OFFSET_Y,
