...
static void on_boot_click_185(void *ud)
{
    (void)ud;
...
static void init_i2c(ws185_ctx_t *ctx)
{
...
static void init_tca9554(ws185_ctx_t *ctx)
{
...
static void init_st77916_display(ws185_ctx_t *ctx)
{
    ...
    ret = esp_lcd_panel_io_rx_param(ctx->panel_io, lcd_cmd, register_data, param_size);
    if (ret == ESP_OK) {
    ...
    if (esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)QSPI_LCD_HOST,
            &io_config, &ctx->panel_io) != ESP_OK) {
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_st77916(ctx->panel_io, &panel_config, &ctx->panel));
...
static void init_buttons(ws185_ctx_t *ctx)
{
    init_buttons_custom();

    board_btn_gpio_cfg_t boot_cfg = { .gpio_num = BOOT_BUTTON_GPIO };
    ctx->boot_button = board_btn_create_gpio(&boot_cfg);
    board_btn_on_click(ctx->boot_button, on_boot_click_185, ctx);
}
...
{
    ws185_ctx_t *ctx = (ws185_ctx_t *)self;
    if (!ctx->codec) {
...
{
    ws185_ctx_t *ctx = calloc(1, sizeof(ws185_ctx_t));
    if (!ctx) return NULL;
