...
static inline void set_cs_1(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.cs, 1); }
static inline void set_cs_0(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.cs, 0); }
static inline void set_dc_1(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.dc, 1); }
static inline void set_dc_0(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.dc, 0); }
static inline void set_rst_1(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.rst, 1); }
static inline void set_rst_0(epd_397_ctx_t *ctx) { gpio_set_level((gpio_num_t)ctx->spi_data.rst, 0); }

static void spi_send_byte(epd_397_ctx_t *ctx, uint8_t data)
{
...
static void epd_send_data(epd_397_ctx_t *ctx, uint8_t data)
{
    set_dc_1(ctx);
    set_cs_0(ctx);
...
static void epd_send_command(epd_397_ctx_t *ctx, uint8_t command)
{
    set_dc_0(ctx);
    set_cs_0(ctx);
...
static void epd_write_bytes(epd_397_ctx_t *ctx, const uint8_t *buf, int len)
{
    set_dc_1(ctx);
    set_cs_0(ctx);
    ...
    while (remaining > 0) {
        ...
        spi_transaction_t t = { .length = 8 * chunk_size, .tx_buffer = buf + offset };
        esp_err_t ret = spi_device_polling_transmit(ctx->spi, &t);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "SPI transmit failed at offset %d, chunk %d: %s",
                     offset, chunk_size, esp_err_to_name(ret));
            break;
        }
...
static void read_busy(epd_397_ctx_t *ctx)
{
...
static void epd_set_windows(epd_397_ctx_t *ctx, uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye)
{
    epd_send_command(ctx, 0x44);
    epd_send_data(ctx, (xs * 8) & 0xFF);
    epd_send_data(ctx, ((xs * 8) >> 8) & 0xFF);
    epd_send_data(ctx, (xe * 8) & 0xFF);
    epd_send_data(ctx, ((xe * 8) >> 8) & 0xFF);
...
static void epd_set_cursor(epd_397_ctx_t *ctx, uint16_t xs, uint16_t ys)
{
...
static void epd_turn_on_display(epd_397_ctx_t *ctx)
{
...
static void epd_turn_on_display_part(epd_397_ctx_t *ctx)
{
    epd_send_command(ctx, 0x22);
    epd_send_data(ctx, 0xFF);
    epd_send_command(ctx, 0x20);
    read_busy(ctx);
}

static void epd_init(epd_397_ctx_t *ctx)
{
    set_rst_1(ctx);
    ...
    epd_send_command(ctx, 0x0C);
    epd_send_data(ctx, 0xAE);
    epd_send_data(ctx, 0xC7);
    epd_send_data(ctx, 0xC3);
    epd_send_data(ctx, 0xC0);
    epd_send_data(ctx, 0x80);

    epd_send_command(ctx, 0x01);
    epd_send_data(ctx, (ctx->height - 1) % 256);
    epd_send_data(ctx, (ctx->height - 1) / 256);
    epd_send_data(ctx, 0x02);
    ...
    epd_send_data(ctx, 0x00);
    epd_send_data(ctx, (ctx->width - 1) % 256);
    epd_send_data(ctx, (ctx->width - 1) / 256);
...
static void epd_clear(epd_397_ctx_t *ctx)
{
    memset(ctx->buffer, 0xff, ctx->spi_data.buffer_len);
}

static void epd_display(epd_397_ctx_t *ctx)
{
...
static void epd_init_partial(epd_397_ctx_t *ctx)
{
    set_rst_1(ctx);
    vTaskDelay(pdMS_TO_TICKS(50));
    set_rst_0(ctx);
...
static void epd_display_part(epd_397_ctx_t *ctx)
{
    epd_send_command(ctx, 0x24);
    assert(ctx->buffer);
    epd_write_bytes(ctx, ctx->buffer, 48000);
    epd_turn_on_display_part(ctx);
}

static void epd_draw_color_pixel(epd_397_ctx_t *ctx, uint16_t x, uint16_t y, uint8_t color)
{
    ...
    uint16_t index = y * ctx->width / 8 + (x >> 3);
...
static void spi_gpio_init(epd_397_ctx_t *ctx)
{
...
static void spi_port_init(epd_397_ctx_t *ctx)
{
...
{
    assert(disp != NULL);
    epd_397_ctx_t *ctx = (epd_397_ctx_t *)lv_display_get_user_data(disp);
...
display_t *custom_epd_397_display_create(const custom_epd_spi_t *spi_cfg, int width, int height)
{
    epd_397_ctx_t *ctx = calloc(1, sizeof(epd_397_ctx_t));
    if (!ctx) return NULL;
