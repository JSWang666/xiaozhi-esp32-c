...
{
    xingzhi_ctx_t *ctx = (xingzhi_ctx_t *)ud;
    if (!ctx->codec) return;
...
static void *get_audio_codec(board_desc_t *self)
{
    ...
    if (!ctx->codec) {
        ctx->codec = es8311_codec_create(ctx->i2c_bus, I2C_NUM_0,
            ...
            AUDIO_CODEC_I2C_PA_EN, AUDIO_CODEC_ES8311_ADDR, false, false);
    }
    return ctx->codec;
}

static void *get_display(board_desc_t *self) { return ((xingzhi_ctx_t *)self)->display; }
