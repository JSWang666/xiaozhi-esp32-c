...
static gpio_num_t AUDIO_I2S_GPIO_DIN_PIN = AUDIO_I2S_GPIO_DIN_1;
static gpio_num_t AUDIO_CODEC_PA_PIN_VAL = AUDIO_CODEC_PA_PIN_1;
static gpio_num_t QSPI_PIN_NUM_LCD_RST_PIN = QSPI_PIN_NUM_LCD_RST_1;
...
static uint8_t detect_pcb_version(i2c_master_bus_handle_t i2c_bus)
{
    ...
    bool codec_alive = (i2c_master_probe(i2c_bus, 0x18, 100) == ESP_OK);
    ...
    if (codec_alive) {
        ...
        vTaskDelay(pdMS_TO_TICKS(50));
        codec_alive = (i2c_master_probe(i2c_bus, 0x18, 100) == ESP_OK);
        if (codec_alive) {
            ESP_LOGI(TAG, "PCB version V1.2");
            pcb_version = 1;
            AUDIO_I2S_GPIO_DIN_PIN = AUDIO_I2S_GPIO_DIN_2;
            AUDIO_CODEC_PA_PIN_VAL = AUDIO_CODEC_PA_PIN_2;
            QSPI_PIN_NUM_LCD_RST_PIN = QSPI_PIN_NUM_LCD_RST_2;
        } else {
...
static void init_i2c(vocat_ctx_t *ctx)
{
...
static void init_display(vocat_ctx_t *ctx, uint8_t pcb_version)
{
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = QSPI_PIN_NUM_LCD_RST_PIN,
...
static void init_buttons(vocat_ctx_t *ctx)
{
...
static bool get_temperature(board_desc_t *self, float *temperature)
{
    (void)self;
    if (!temp_sensor) return false;
    return temperature_sensor_get_celsius(temp_sensor, temperature) == ESP_OK;
}
...
static void *get_audio_codec(board_desc_t *self)
{
    ...
    if (!ctx->codec) {
        ctx->codec = box_codec_create(ctx->i2c_bus,
            ...
            AUDIO_I2S_GPIO_DOUT, AUDIO_I2S_GPIO_DIN_PIN,
            AUDIO_CODEC_PA_PIN_VAL, AUDIO_CODEC_ES8311_ADDR, AUDIO_CODEC_ES7210_ADDR,
...
{
    return ((vocat_ctx_t *)self)->display;
}
...
{
    vocat_ctx_t *ctx = calloc(1, sizeof(vocat_ctx_t));
    if (!ctx) return NULL;
    ...
    ctx->base.get_temperature = get_temperature;
    ctx->base.destroy = destroy;

    init_i2c(ctx);
    uint8_t pcb_version = detect_pcb_version(ctx->i2c_bus);
    init_spi();
    init_display(ctx, pcb_version);
    init_buttons(ctx);
