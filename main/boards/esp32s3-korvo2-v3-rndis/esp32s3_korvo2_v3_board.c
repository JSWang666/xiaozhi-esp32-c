...
{
    korvo2_rndis_ctx_t *ctx = (korvo2_rndis_ctx_t *)ud;
    if (!ctx->codec) return;
...
static void init_i2c(korvo2_rndis_ctx_t *ctx)
{
...
static void init_tca9554(korvo2_rndis_ctx_t *ctx)
{
...
static void enable_lcd_cs(korvo2_rndis_ctx_t *ctx)
{
    if (ctx->io_expander)
...
static void init_buttons(korvo2_rndis_ctx_t *ctx)
{
...
static void init_ili9341_display(korvo2_rndis_ctx_t *ctx)
{
...
#else
static void init_st7789_display(korvo2_rndis_ctx_t *ctx)
{
    esp_lcd_panel_io_handle_t panel_io = NULL;
    esp_lcd_panel_handle_t panel = NULL;
...
{
    (void)self;
    return "esp32s3-korvo2-v3-rndis";
}

static void *get_audio_codec(board_desc_t *self)
{
    korvo2_rndis_ctx_t *ctx = (korvo2_rndis_ctx_t *)self;
    if (!ctx->codec) {
...
{
    return ((korvo2_rndis_ctx_t *)self)->display;
}
...
{
    korvo2_rndis_ctx_t *ctx = calloc(1, sizeof(korvo2_rndis_ctx_t));
    if (!ctx) return NULL;

    ctx->base.kind = BOARD_KIND_RNDIS;
    ...
    ESP_LOGI(TAG, "Initializing esp32s3_korvo2_v3 Board (RNDIS)");
    init_i2c(ctx);
    init_tca9554(ctx);
    init_spi();
    init_buttons(ctx);
