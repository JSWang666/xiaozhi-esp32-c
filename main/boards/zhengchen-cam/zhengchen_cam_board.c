...
static zhengchen_cam_ctx_t *g_zcb_ctx;

static void wrapped_enable_output(audio_codec_t *codec, bool enable)
{
    if (g_zcb_ctx && g_zcb_ctx->orig_ops && g_zcb_ctx->orig_ops->enable_output)
        g_zcb_ctx->orig_ops->enable_output(codec, enable);

    if (g_zcb_ctx && g_zcb_ctx->pca9557)
        pca9557_set_output_state(g_zcb_ctx->pca9557, 1, enable ? 1 : 0);
}
...
{
    zhengchen_cam_ctx_t *ctx = (zhengchen_cam_ctx_t *)ud;
    if (!ctx->codec) return;
...
static void init_i2c(zhengchen_cam_ctx_t *ctx)
{
...
static void init_spi(zhengchen_cam_ctx_t *ctx)
{
    (void)ctx;
...
static void init_st7789_display(zhengchen_cam_ctx_t *ctx)
{
...
static void init_buttons(zhengchen_cam_ctx_t *ctx)
{
...
static const char *zcb_get_board_type(board_desc_t *self)
{
    (void)self;
    return BOARD_TYPE;
}

static void *zcb_get_led(board_desc_t *self)
{
...
static void *zcb_get_audio_codec(board_desc_t *self)
{
    zhengchen_cam_ctx_t *ctx = (zhengchen_cam_ctx_t *)self;
    if (!ctx->codec) {
        ...
        if (ctx->codec && ctx->codec->ops) {
            ...
            ctx->wrapped_ops.enable_output = wrapped_enable_output;
...
static void *zcb_get_display(board_desc_t *self)
{
    zhengchen_cam_ctx_t *ctx = (zhengchen_cam_ctx_t *)self;
    return ctx->display;
}

static void *zcb_get_backlight(board_desc_t *self)
{
    zhengchen_cam_ctx_t *ctx = (zhengchen_cam_ctx_t *)self;
    if (!ctx->backlight) {
...
{
    zhengchen_cam_ctx_t *ctx = calloc(1, sizeof(zhengchen_cam_ctx_t));
    if (!ctx) return NULL;
    g_zcb_ctx = ctx;

    ctx->base.kind = BOARD_KIND_WIFI;
    ctx->base.get_board_type = zcb_get_board_type;
    ctx->base.get_led = zcb_get_led;
    ctx->base.get_audio_codec = zcb_get_audio_codec;
    ctx->base.get_display = zcb_get_display;
    ctx->base.get_backlight = zcb_get_backlight;
    ctx->base.destroy = zcb_destroy;
    ...
    backlight_restore_brightness(zcb_get_backlight(&ctx->base));
